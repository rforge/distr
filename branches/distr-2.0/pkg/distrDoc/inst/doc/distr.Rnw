...
\subsubsection{Parameter classes}\label{paramclass}
%
...
% -----------------------------------------------------------------------------
\subsubsection[ParamFamily and friends]{\code{ParamFamily} and friends}
% -----------------------------------------------------------------------------
\ifpdf
\begin{figure}[!ht]
\vspace{2ex}
...
\fi
% -----------------------------------------------------------------------------
\subsubsection[ParamFamParameter class]{Class \code{ParamFamParameter}}
% -----------------------------------------------------------------------------
In particular, to cope with partitioned parameters (with main and nuisance
part) and to allow for smooth transformations of the parameter, we introduce
a sub-class \code{ParamFamParameter} to our class \code{Parameter} from 
subsection~\ref{parameterclass}. It has slots \code{main}, \code{nuisance}, 
and \code{trafo}.  

Implementaion of partial influence curves (pICs) is done via slot \code{trafo}. 
pICs arise if we are only  interested is some possibly lower dimensional, smooth 
(not necessarily linear or even coordinate-wise) aspect/transformation 
  $\tau$  of the whole parameter $\theta$.

The nuisance part has its own slot. It arises if there is some unknown parameter,
the value of which is of secondary interest compared to the main aspect
of the parameter, but knowledge of which is necessary for estimation or testing
(e.g. an unknown scale parameter for confidence intervals about an interesting
location parameter).

To get a coherent implementation of the two concepts, nuisance and transformation
$\tau$,  we make the following convention:

The full parameter $\theta$ is first split up coordinate-wise into a main 
parameter $\theta'$ and a nuisance parameter $\theta''$. Without loss of 
generality, we restrict the transformation $\tau$ only to act on the main 
parameter $\theta'$; this is no real restriction, because if we want to 
transform the whole parameter, we only need to assume that nuisance parameter
$\theta''$ be of length $0$.
  
To the implementation:  As we have given the same name \code{trafo} to
several different objects, we have to be a little careful about what type
of \code{trafo} we are talking about; we will try to be clear about this.

Slot \code{trafo} of an object of class \code{ParamFamParameter} can either 
contain a (constant) matrix
 $D_\theta$ or a function
  $\tau\colon \Theta \to \tilde \Theta,\qquad \theta \mapsto \tau(\theta)$ 
   mapping the main parameter
  $\theta'$ to some range $\tilde \Theta$.

If {\em slot value} \code{trafo} is a function, besides the function value 
$\tau(\theta)$, it will also return the corresponding derivative matrix
$D_\theta=\frac{\partial}{\partial \theta}\tau(\theta)$.
More specifically, the return value of this function \code{theta} is a
  ...
  and, correspondingly, we interpet $\tau(\theta)$ as the linear mapping
  $\tau(\theta)=D\,\theta$.\par

 For several classes, \code{trafo} also denotes the accessor function to slots 
 containing information about the transformation. According to the signature, 
 this {\em method} \code{trafo} will return different return value types. 
 For signature
  \begin{itemize}
    ...
    Function $\tau$ then returns the list \code{list(fval, mat)} mentioned above
    
    \item[]\code{ParamFamParameter,missing}: it will just return the
     corresponding matrix.
    \item[]\code{ParamFamily,missing}: being just a wrapper to signature
    \code{ParamFamParameter,missing}, it will do the same.
    \item[]\code{ParamFamily,ParamFamParameter}: it will return the same as 
    signature \code{Estimate,missing}.
  \end{itemize}  
...
\vspace{2ex}
  \begin{center}
    \includegraphics[width=4cm]{BiasType.pdf}%
...
\begin{figure}[htb]
  \begin{center}
    \includegraphics[width=4cm]{BiasType.eps}%
    \caption{\label{figBT}{\footnotesize Inheritance relations and slots of the 
    corresponding \mbox{(sub-)}classes for \code{BiasType} where we do not 
    repeat inherited slots
    }}
...
if (have.distrMod){
    print(MLEstimator(x = x, ParamFamily = G))
    MDEstimator(x = x, ParamFamily = G, distance = KolmogorovDist)
}
@
The results of these computations are objects of the S4-class \code{MCEstimate}
which inherits from the S4-class \code{Estimate}. The definitions are given
...
\end{figure}
\fi
As an example of how a transformation $\tau$ may enter, consider the following
reparametrization in our example: If $\theta=\mbox{(\code{shape}, \code{scale})}$, let 
$\tau(\theta)=(\theta_2^{-1}, \theta_1)=\mbox{(\code{rate},\code{shape})}$,
the parametrization as in B. Ripley's  
\code{fit.distr} (confer \cite{V:R:02}) from package \pkg{MASS}. 
This gives the following transformation
<<mtrafo, eval = TRUE>>=
if (have.distrMod){
mtrafo <- function(x){
...
G2 <- G
trafo(G2) <- mtrafo
(res.Mod <- MLEstimator(x = x, ParamFamily = G2))
}
@
...
<<EstCmp, eval = TRUE>>=
if (have.distrMod){
print(res.Mod, digits = 3, show.details = "minimal")
print(res.MASS, digits = 3)
}
@

The actual behavior of \code{print} and \code{show} for various option
settings is described in detail in the corresponding 
\code{<classname>-class}-help file, e.g. by inspecting \code{?Estimate-Class}.

% -----------------------------------------------------------------------------
\subsection{Confidence Intervals}
% -----------------------------------------------------------------------------
For a unified interface to confidence intervals, we have overloaded method
\code{confint} from package \code{stats}. It now dispatches according to
arguments \code{object} and \code{method}, where upto now we only use the
first one and have only implemented methods for the second being \code{missing}.
Later on, for one type of estimator in the first argument, several methods
can be defined like asymptotic, bootstrap-based, simulation-based,
Cornish-Fisher-based and so on. For the time being, we have implemented 
methods for asymptotic CLT-based confidence intervals for objects of
class \code{Estimate}. The default method (signature \code{object=ANY,method=missing})
just uses \code{stats::confint}. Our method for class \code{Estimate}
uses the asymptotic (co)variance (if present in slot \code{asvar}) 
of the estimator, and then builds a symmetric CLT (i.e. Gaussian) confidence
interval about the estimator value.
In our example this gives
<<CI, eval = TRUE>>=
(ci.Mod <- confint(res.Mod))
@
For comparison, we add a convenient (albeit wrong) S3-method for \code{vcov} 
---wrong as in general the covariance matrix of an estimator will not be
diagonal; but for our coordinate-wise confidence intervals this does not 
matter, as only the diagonal entries determine the confidence bounds.
...
vcov.fitdistr <- function(object, ...){
     ...
     v}
(ci.MASS <- confint(res.MASS))
}
@
or again, shortly:
<<CICmp2, eval = TRUE>>=
if (have.distrMod){
print(ci.Mod, digits = 3, show.details = "minimal")
print(ci.MASS, digits = 3)
}
@
\code{confint} will return objects of (S4-)class \code{Confint}, so that
the user can easily manipulate the output within {\sf R}. The class structure
is depicted in Figure~\ref{figConf}.
\ifpdf
\begin{figure}[!ht]
\vspace{2ex}
  \begin{center}
    \includegraphics[width=4cm]{Confint.pdf}%
    \caption{\label{figConf}{\footnotesize Inheritance relations and slots of the 
    corresponding \mbox{(sub-)}classes for \code{Confint} where we do not
    repeat inherited slots
    }}
  \end{center}
\vspace{-4ex}
...
\begin{figure}[htb]
  \begin{center}
    \includegraphics[width=4cm]{Confint.eps}%
    \caption{\label{figConf}{\footnotesize Inheritance relations and slots of the 
    corresponding \mbox{(sub-)}classes for \code{Confint} where we do not 
    repeat inherited slots
    }}
...
\subsection{Masking}
% -----------------------------------------------------------------------------
To ensure positive (semi)-definiteness of matrices like covariances, we have 
introduced classes \code{PosSemDefSymmMatrix} and \code{PosDefSymmMatrix}; for 
these the square root can be defined. More specifically we have overloaded
...
\end{itemize}
\subsection[Options for distrMod]{Options for \pkg{distrMod}}\label{distrModOptions}
...
\begin{itemize}
  \item \code{show.details} controls the detailedness of output by methods
  \code{print} and \code{show}. Allowed values are \code{"maximal"}, 
  \code{"medium"}, and \code{"minimal"}; defaults to "maximal".
\end{itemize}
...
\bibitem{K:R:S:04}
Kohl M., Ruckdeschel P. and Stabla T. (2004):
...
\newblock {\em {Modern Applied Statistics with
  S. Fourth Edition.\/}} 
