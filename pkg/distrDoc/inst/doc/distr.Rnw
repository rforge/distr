...
option
\code{WarningArith} to \code{FALSE} (see section~\ref{options}).\medksip\\
%
...
\vspace{2ex}
  \begin{center}
    \includegraphics[width=8cm]{BiasType.pdf}%
...
\begin{figure}[htb]
  \begin{center}
    \includegraphics[width=7.5cm]{BiasType.eps}%
    \caption{\label{figBT}{\footnotesize Inheritance relations and slots of the 
    corresponding \mbox{(sub-)}classes for \code{BiasType} where we do not 
    repeat inherited slots
    }}
...
@
The results of these computations are objects of the S3-class \code{MCEstimate}
which inherits from the S3-class \code{Estimate} and consists of a \code{list}
with the two components
\begin{description}
  \item[estimate] minimum criterion parameter estimate
  \item[criterion] minimum value of the criterion
\end{description}
The printing is done via \code{print.Estimate} which prints only the estimate
component of \code{MCEstimate}.
% -----------------------------------------------------------------------------
